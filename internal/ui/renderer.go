...
func (r *Renderer) renderCPU(data interface{}) {
	...
	if cpuInfo, ok := data.(*cpu.CPUInfo); ok {
		fmt.Printf("\033[0m\033[36m%s:\033[0m %s\n", r.config.T("cpu.model"), cpuInfo.Model)
		fmt.Printf("\033[0m\033[36m%s:\033[0m %d %s, %d %s\n", 
...
func (r *Renderer) renderMemory(data interface{}) {
	...
	if memInfo, ok := data.(*mem.MemoryInfo); ok {
		...
		fmt.Printf("\033[0m\033[36m%s:\033[0m %s / %s %s \033[38;5;154m%.1f%%\033[0m\n",
	...
	if disks, ok := data.([]disk.DiskInfo); ok {
		...
		for _, d := range disks {
			if strings.HasPrefix(d.Filesystem, "/dev/") && count < 3 {
				...
				fmt.Printf("\033[0m\033[36m%s:\033[0m %s (%s)\n", 
				...
				fmt.Printf("\033[0m\033[36m%s:\033[0m %s / %s %s \033[38;5;216m%.1f%%\033[0m\n",
	...
	if networks, ok := data.([]net.NetworkInfo); ok {
		...
		for _, net := range networks {
			if net.Status == "UP" && net.Interface != "lo" && count < 2 {
				...
				fmt.Printf("\033[0m\033[36m%s:\033[0m %s \033[38;5;165m%.1f%%\033[0m\n",
...
func (r *Renderer) renderFooter() {
	fmt.Printf("\033[0m\033[90mPress 'q' to quit | Auto-refresh every %d seconds\033[0m\n", 
